...
import { FaCheck } from 'react-icons/fa'; // 체크 아이콘 불러오기

const Consent = () => {
  const [allChecked, setAllChecked] = useState(false); // 전체 동의 체크 상태
  const [termsChecked, setTermsChecked] = useState(false);
  const [privacyChecked, setPrivacyChecked] = useState(false);
  const [marketingChecked, setMarketingChecked] = useState(false); // 선택 동의
