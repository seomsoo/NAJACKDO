...
const iconSettings: { [key: string]: { icon: JSX.Element, color: string } } = {
  "대출 신청": { icon: <IoIosLeaf size={24} color="#79AC78" />, color: "#79AC78" },
  "대출 가능": { icon: <TbBellRingingFilled size={24} color="#5F6F52" />, color: "#5F6F52" },
...
const Alarm = ({ type, message, time }: AlarmProps) => {
  ...
  return (
    <div className="mx-[25px] my-5 flex flex-row">
      {icon}&nbsp;
      <div className="flex flex-col gap-1">
        <p className="text-[15px] col-span-2" style={{ color: color }}>{type}</p>
        <p className="row-span-2 col-span-2 text-[12px]">{message}</p>
        <p className="col-span-2 text-[10px]">{timeDifference}</p> {/* 시간 차이 표시 */}
      </div>
    </div>
  );
};
